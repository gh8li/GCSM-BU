...
// __global__ void ours_write_initial_partial_results(
...
//     uint8_t u0 = get_query_vertex(order[warp_id], 0);
//     uint8_t u1 = get_query_vertex(order[warp_id], 1);
//     uint8_t edge_idx = get_query_edge_idx(u0, u1);
...
// __global__ void ours_enumerate(
...
//     RelationsGPUAllVersions data_graph_all_versions,
...
//             atomicAdd(effective_num_dptr, 1);
//         }
...
// //     }
// // #endif
//                             total_nbr_count[warp_id][cur_depth] = get_nbr_size(data_graph_all_versions.GetDataGraphByIdx((size_t)matching_order_idx), pre_qe_idx, pre_dv);
...
// //     }
// // #endif
//                         temp_nbr = get_nbr(data_graph_all_versions.GetDataGraphByIdx((size_t)matching_order_idx), pre_qe_idx, pre_dv, nbr_idx);
...
//                             found = find_nbr(data_graph_all_versions.GetDataGraphByIdx((size_t)matching_order_idx), cur_pre_qe_idx, cur_pre_dv, temp_nbr);
...
// // #endif
//                                 atomicAdd(effective_num_dptr, temp_found_nbr_count);
//                             }
...
// __global__ void OursGetNumTree(
...
//             max_num_matches[i] *= data_graph_all_versions.GetDataGraphByIdx((size_t)order_idx).sizes_[pre_qe_idx][v];
...
// __global__ void oursEnumerateCartesianProductTree(
//     const unsigned long long res,
//     const unsigned long long res_size,
//     const unsigned long *max_num_previous_matches,
...
//     for (auto i = global_warp_id; i < DIV_CEIL(*total_i, WARP_SIZE); i += num_warps)
//     {
...
//         if (found)
//         {
//             res_index = lower_bound(max_num_previous_matches + 1, res_size, ii + 1);
...
//             res_index_index = ii - max_num_previous_matches[res_index];
//         }
...
//                 RelationsGPU &cur_graph = data_graph_all_versions.GetDataGraphByIdx((size_t)order_idx);

//                 map_v = cur_graph.nbrs_[pre_qe_idx][v][res_index_index % cur_graph.sizes_[pre_qe_idx][v]];
//                 res_index_index /= cur_graph.sizes_[pre_qe_idx][v];
